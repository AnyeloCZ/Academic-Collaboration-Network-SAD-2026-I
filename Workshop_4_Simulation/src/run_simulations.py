...
def sample_students_from_custom(custom_students: pd.DataFrame, n: int, rng: np.random.Generator) -> pd.DataFrame:
...
def get_students(n: int, rng: np.random.Generator, custom_students: pd.DataFrame | None = None) -> pd.DataFrame:
    if custom_students is not None:
        return sample_students_from_custom(custom_students, n, rng)
...
def simulate_process_scenarios(config: dict, custom_students: pd.DataFrame | None = None) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for scenario in config["process_scenarios"]:
        ...
        for trial in range(trials):
            students = get_students(n, rng, custom_students)
...
def simulate_behavior_scenarios(config: dict, custom_students: pd.DataFrame | None = None) -> tuple[pd.DataFrame, pd.DataFrame]:
...
def simulate_sensitivity(config: dict, custom_students: pd.DataFrame | None = None) -> pd.DataFrame:
    ...
    for adoption in config["sensitivity"]["adoption_rates"]:
        for threshold in config["sensitivity"]["isolation_thresholds"]:
            ...
            ts, _ = simulate_behavior_scenarios(local, custom_students)
            final = ts.iloc[-1]
            rows.append(
                {
                    "initial_adoption": adoption,
...
def main() -> None:
    ensure_dirs()
    parser = argparse.ArgumentParser(description="Run Workshop 4 ACN simulations.")
    parser.add_argument("--seed", type=int, help="Use a specific random seed for reproducible alternative data.")
    parser.add_argument("--randomize", action="store_true", help="Use a different random seed on every run.")
    parser.add_argument("--students", type=str, help="CSV file with custom student profiles.")
    parser.add_argument("--ignore-custom", action="store_true", help="Ignore data/custom_students.csv if it exists.")
    ...
    process_trials, process_summary = simulate_process_scenarios(config, custom_students)
    print("2/6 Running behavior-oriented simulation...")
    behavior_timeseries, behavior_summary = simulate_behavior_scenarios(config, custom_students)
