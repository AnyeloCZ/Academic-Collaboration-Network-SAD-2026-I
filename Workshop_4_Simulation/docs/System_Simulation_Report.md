...
The optimized ACN scenario reduced median group formation time by 91.1% compared with the WhatsApp-based AS-IS process, reduced isolated students by 64.2% in the process model, and kept p95 matching latency at 1.25 seconds under the 500-student exam-peak scenario. In the behavior model, the isolation-priority balancing loop reduced isolated students by 91.2% and increased collaboration density by 920.2% compared with informal self-selection.

The simulation validates the main design decisions from previous workshops: a dedicated matching engine is feasible for the target cohort size, centralized workspaces reduce coordination friction, and centrality monitoring materially improves equity. The main challenged assumption is adoption: the low-adoption scenario still leaves 12 isolated students after 12 weeks, confirming Workshop 3's low-adoption risk as the highest operational priority.
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| AS_IS_WhatsApp | 200 | 0.39 | 4.52 | N/A | 121.9 | 0.30 | 0.38 |
| ACN_Baseline | 200 | 0.90 | 0.39 | 0.27 | 72.8 | 0.66 | 0.84 |
| ACN_Optimized_B1 | 200 | 0.98 | 0.40 | 0.21 | 43.7 | 0.66 | 0.87 |
| ACN_LMS_Outage_Fallback | 200 | 0.72 | 0.54 | 0.31 | 98.6 | 0.66 | 0.74 |
| ACN_Exam_Peak_500 | 500 | 0.93 | 0.38 | 1.25 | 141.6 | 0.66 | 0.85 |

The AS-IS process remains slow and exclusionary because student discovery depends on existing social connections. The optimized ACN scenario compresses formation time from 4.52 days to 0.40 days and improves satisfaction to 0.87. The LMS outage scenario remains operational but suffers a measurable decline in schedule compatibility and satisfaction, validating the fallback strategy while showing that integration reliability still matters.

### Behavior-Oriented Simulation

| Scenario | Adoption | Density | Isolated | Gini | Giant component | Satisfaction |
| --- | --- | --- | --- | --- | --- | --- |
| AS_IS_Self_Selection | 0.00 | 0.011 | 34 | 0.44 | 0.80 | 0.47 |
| ACN_No_B1 | 0.43 | 0.022 | 19 | 0.47 | 0.88 | 0.61 |
| ACN_With_B1 | 0.85 | 0.112 | 3 | 0.20 | 0.98 | 0.77 |
| ACN_Low_Adoption_20 | 0.38 | 0.027 | 12 | 0.50 | 0.93 | 0.62 |

The behavior model shows nonlinear network growth. Once platform adoption and satisfaction reinforce each other, density increases quickly and the giant component covers most of the cohort. The B1 isolation-priority loop reduces centrality inequality from 0.44 in the AS-IS scenario to 0.20, indicating a more equitable distribution of collaboration opportunities.

### Sensitivity Analysis

The best sensitivity outcome occurs at initial adoption 0.9 with threshold 3, leaving 0 isolated students. The worst outcome occurs at initial adoption 0.4 with threshold 2, leaving 22 isolated students. This confirms that adoption is not a cosmetic metric; it changes the topology of the collaboration network.
